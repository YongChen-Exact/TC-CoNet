...
from TCCoNet.evaluation.add_mean_dice_to_json import foreground_mean
from TCCoNet.evaluation.evaluator import evaluate_folder
from TCCoNet.evaluation.model_selection.ensemble import ensemble
from TCCoNet.paths import network_training_output_dir
import numpy as np
from subprocess import call
from TCCoNet.postprocessing.consolidate_postprocessing import consolidate_folds, collect_cv_niftis
from TCCoNet.utilities.folder_names import get_output_folder_name
from TCCoNet.paths import default_cascade_trainer, default_trainer, default_plans_identifier
...
def main():
    import argparse
    parser = argparse.ArgumentParser(usage="This is intended to identify the best TransNorm based on the five fold "
    ...
    parser.add_argument("-tr", type=str, required=False, default=default_trainer,
                        help="TCCoNetTrainer class. Default: %s" % default_trainer)
    parser.add_argument("-ctr", type=str, required=False, default=default_cascade_trainer,
                        help="TCCoNetTrainer class for cascade TransNorm. Default: %s" % default_cascade_trainer)
    ...
    for t in tasks:
        # first collect pure TransNorm performance
        results = {}
        all_results = {}
        valid_models = []
        for m in models:
            ...
            if not isdir(output_folder):
                raise RuntimeError("Output folder for TransNorm %s is missing, expected: %s" % (m, output_folder))
            ...
            if disable_postprocessing:
                ...
                # we need cv_niftis_postprocessed to know the single TransNorm performance. And we need the
                ...
                if not isfile(postprocessing_json) or not isdir(cv_niftis_folder):
                    print("running missing postprocessing for %s and TransNorm %s" % (id_task_mapping[t], m))
        ...
        for k, v in results.items():
            if v == best:
                print("%s submit TransNorm %s" % (id_task_mapping[t], k), v)
                best_model = k
                ...
                if k.startswith("ensemble"):
                    ...
                    predict_str += "TCCoNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr " + tr + " -ctr " + trc + " -m " + m1 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"
                    predict_str += "TCCoNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL2 -tr " + tr + " -ctr " + trc + " -m " + m2 + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"

                    if not disable_postprocessing:
                        predict_str += "TCCoNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER -pp " + join(network_training_output_dir, "ensembles", id_task_mapping[t], k, "postprocessing.json") + "\n"
                    else:
                        predict_str += "TCCoNet_ensemble -f OUTPUT_FOLDER_MODEL1 OUTPUT_FOLDER_MODEL2 -o OUTPUT_FOLDER\n"
                else:
                    predict_str += "TCCoNet_predict -i FOLDER_WITH_TEST_CASES -o OUTPUT_FOLDER_MODEL1 -tr " + tr + " -ctr " + trc + " -m " + k + " -p " + pl + " -t " + \
                                   id_task_mapping[t] + "\n"
                print(predict_str)
