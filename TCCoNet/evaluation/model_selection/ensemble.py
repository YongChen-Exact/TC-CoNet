...
from TCCoNet.paths import network_training_output_dir, preprocessing_output_dir
...
def ensemble(training_output_folder1, training_output_folder2, output_folder, task, validation_folder, folds, allow_ensembling: bool = True):
    ...
    for f in folds:
        ...
        if not isdir(validation_folder_net1):
            raise AssertionError("Validation directory missing: %s. Please rerun validation with `TCCoNet_train CONFIG TRAINER TASK FOLD -val --npz`" % validation_folder_net1)
        if not isdir(validation_folder_net2):
            raise AssertionError("Validation directory missing: %s. Please rerun validation with `TCCoNet_train CONFIG TRAINER TASK FOLD -val --npz`" % validation_folder_net2)
        ...
        if not isfile(join(validation_folder_net1, 'summary.json')):
            raise AssertionError("Validation directory incomplete: %s. Please rerun validation with `TCCoNet_train CONFIG TRAINER TASK FOLD -val --npz`" % validation_folder_net1)
    ...
    if allow_ensembling and not isfile(join(output_folder_base, "postprocessing.json")):
        ...
        # here because things may have changed and may also be too inconsistent between the two TransClawUNet
