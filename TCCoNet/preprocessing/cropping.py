...
def get_case_identifier(case):
    # print(case)
    case_identifier = case[0].split("/")[-1].split(".nii.gz")[0][-23:-5]
    print(case_identifier+" get")
    return case_identifier


def get_case_identifier_from_npz(case):
    case  = case.replace("\\","/")
    case_identifier = case.split("/")[-1][-27:-4]
    print(case_identifier+" npz")
    return case_identifier
...
def get_patient_identifiers_from_cropped_files(folder):
    ...
    return [i.split("\\")[-1][-26:-4] for i in subfiles(folder, join=True, suffix=".npz")]
...
class ImageCropper(object):
    ...
    def run_cropping(self, list_of_files, overwrite_existing=False, output_folder=None):
        ...
        for j, case in enumerate(list_of_files):
            ...
            print("identifier:"+case_identifier)
