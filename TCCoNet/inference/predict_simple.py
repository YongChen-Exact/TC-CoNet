...
from TCCoNet.inference.predict import predict_from_folder
from TCCoNet.paths import default_plans_identifier, network_training_output_dir, default_cascade_trainer, default_trainer
...
def main():
    ...
    parser.add_argument('-tr', '--trainer_class_name',
                        help='Name of the TCCoNetTrainer used for 2D U-Net, full resolution 3D U-Net and low resolution '
    ...
    parser.add_argument('-m', '--TransNorm', help="2d, 3d_lowres, 3d_fullres or 3d_cascade_fullres. Default: 3d_fullres",
    ...
    parser.add_argument('-z', '--save_npz', required=False, action='store_true',
                        help="use this if you want to ensemble these predictions with those of other models. Softmax "
                             ...
                             "merged between output_folders with TCCoNet_ensemble_predictions")

    parser.add_argument('-l', '--lowres_segmentations', required=False, default='None',
                        help="if TransNorm is the highres stage of the cascade then you can use this folder to provide "
    ...
    # we need to catch the case where TransNorm is 3d cascade fullres and the low resolution folder has not been set.
    ...
    if model == "3d_cascade_fullres" and lowres_segmentations is None:
        ...
        assert isdir(model_folder_name), "TransNorm output folder not found. Expected: %s" % model_folder_name
    ...
    model_folder_name = join(network_training_output_dir, model, task_name, trainer + "__" +
                              args.plans_identifier)
    print("using TransNorm stored in ", model_folder_name)
    assert isdir(model_folder_name), "TransNorm output folder not found. Expected: %s" % model_folder_name
