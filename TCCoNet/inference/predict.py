...
from TCCoNet.inference.segmentation_export import save_segmentation_nifti_from_softmax, save_segmentation_nifti
...
from TCCoNet.postprocessing.connected_components import load_remove_save, load_postprocessing
from TCCoNet.training.model_restore import load_model_and_checkpoint_files
from TCCoNet.training.network_training.TCCoNetTrainer import TCCoNetTrainer
from TCCoNet.training.network_training.TCCoNetTrainerV3 import TCCoNetTrainerV3
from TCCoNet.utilities.one_hot_encoding import to_one_hot
...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    assert isinstance(trainer, TCCoNetTrainer) or isinstance(trainer, TCCoNetTrainerV3)
    q = Queue(1)
    processes = []
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  ...
                  all_in_gpu=False, step_size=0.5, checkpoint_name="model_epoch_930",
                  segmentation_export_kwargs: dict = None):
    """
    :param segmentation_export_kwargs:
    :param model: folder where the TransNorm is saved, must contain fold_x subfolders
    ...
    :param mixed_precision: if None then we take no action. If True/False we overwrite what the TransNorm has in its init
    :return:
    """
    ...
    if isfile(pp_file):
        ...
        print("WARNING! Cannot run postprocessing because the postprocessing file is missing. Make sure to run "
              "consolidate_folds in the output folder of the TransNorm first!\nThe folder you need to run this in is "
              "%s" % model)
...
def check_input_folder_and_return_caseIDs(input_folder, expected_num_modalities):
    print("This TransNorm expects %d input modalities for each image" % expected_num_modalities)
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    assert isfile(join(model, "plans.pkl")), "Folder with saved TransNorm weights must contain a plans.pkl file"
...
if __name__ == "__main__":
    ...
    parser.add_argument('-m', '--model_output_folder',
                        help='TransNorm output folder. Will automatically discover the folds '
                             'that were '
    ...
    parser.add_argument('-f', '--folds', nargs='+', default='None', help="folds to use for prediction. Default is None "
                                                                         ...
                                                                         "automatically in the TransNorm output folder")
    ...
    parser.add_argument('-l', '--lowres_segmentations', required=False, default='None', help="if TransNorm is the highres "
