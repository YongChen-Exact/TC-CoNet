    ...
    def __init__(self,
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(
                ...
                shift_size=[0, 0, 0] if (i % 2 == 0) else self.shift_size,  # 第一个块不滑，第二个块滑动
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                drop=drop,
                attn_drop=attn_drop,
...
class PPE(nn.Module):
    ...
    def __init__(self, patch_size=4, in_chans=4, embed_dim=96, norm_layer=None):
        ...
        self.conv1 = nn.Conv3d(in_chans, embed_dim // 2, kernel_size=7, stride=(1, 2, 2), padding=(3, 3, 3), bias=False)
        self.bn1 = nn.BatchNorm3d(embed_dim // 2)
        self.relu = nn.GELU()
        self.proj1 = project(embed_dim // 2)

        self.conv2 = nn.Conv3d(embed_dim // 2, embed_dim, kernel_size=3, stride=(1, 2, 2), padding=1)
...
class TCCoNet(SegmentationNetwork):
    ...
    def __init__(self, crop_size=[64, 128, 128],
        ...
        self.decoder3 = UpBlock(in_channels=embed_dim * 8, out_channels=embed_dim * 4, kernel_size=3,
                                ...
                                upsample_stride=[2, 2, 2], has_padding=True)
        ...
        self.decoder1 = UpBlock(in_channels=embed_dim * 2, out_channels=embed_dim, kernel_size=3,
                                upsample_kernel_size=[1, 2, 2], upsample_stride=[1, 2, 2])
